...
export const ConfirmPayment: FC<ConfirmPaymentProps> = ({ disabled, id, label, isMobile = false }) => {
	const [confirmationDetails, setConfirmationDetails] = useState<ConfirmPaymentType>(defaultValuesConfirmPayment)
	...
	return (
		<Dialog>
			...
			<DialogContent className="[&>.Icon]:hidden p-5 rounded-[14px] max-w-[42.5rem] w-full gap-[1.875rem]">
				<DialogTitle className="text-[1.375rem] font-normal text-lynch-400">Commission a recevoir</DialogTitle>
				<div className="flex flex-col gap-3">
					<div className="flex lg:flex-row flex-col items-center gap-4">
						...
						<div className="flex flex-col items-start gap-3 w-full text-lynch-400">
							...
							<DatePicker disabled onChange={() => {}} value={confirmationDetails.dateOfReception} />
						</div>
					</div>
					<div className="flex lg:flex-row flex-col items-center gap-4">
						<Select
							disabled
							onChange={() => {}}
							options={[{ key: confirmationDetails.transmitter, label: confirmationDetails.transmitter }]}
							label="Nom émetteur"
							value={confirmationDetails.transmitter}
						/>
						<Input disabled name="amount" onChange={() => {}} value={confirmationDetails.amount} label="Amount" />
					</div>
					<div className="flex lg:flex-row flex-col items-center gap-4">
						<div className="flex flex-col items-start gap-3 w-full text-lynch-400">
							...
							<span
								...
								<Image
									...
									height={48}
									alt="Word"
									src={confirmationDetails.document.fileName.includes("pdf") ? "/word-icon.png" : "/pdf-icon.png"}
								/>
								<Label label={confirmationDetails.document.fileName} className="text-lynch-500 text-base font-normal" />
							</span>
						</div>
					</div>
