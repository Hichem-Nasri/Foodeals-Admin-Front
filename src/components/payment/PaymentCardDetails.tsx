...
import { Banknote, Building, Calendar, CalendarClock, CirclePercent, Coins, Frame, HandCoins } from "lucide-react"
...
export const PaymentCardDetails: React.FC<PaymentCardDetailsProps> = ({ payment }): JSX.Element => {
	const dataArray = [
		{
			label: payment.type,
			icon: Building,
		...
		},
		{
			...
			icon: Banknote,
			className: payment.status !== PaymentStatusType.PAID ? "text-red-500 bg-red-50" : "text-green-500 bg-green-50",
		},
		{
			...
			<div className="flex justify-between gap-[0.375rem]">
				<div className="flex gap-[0.375rem]">
					<Avatar className="size-[2.875rem] shrink-0">
						...
						<AvatarFallback>{payment.store.name[0].toUpperCase()}</AvatarFallback>
					</Avatar>
					<div className="flex flex-col gap-1">
						<Label label={payment.store.name} className="text-sm font-normal text-lynch-950" />

						<div className="flex items-center gap-2 text-lynch-500">
							<CalendarClock size={18} />
							<Label label={payment.date.toLocaleDateString()} className="text-xs font-medium text-lynch-500" />
						</div>
					</div>
				</div>
			...
			<div className="flex flex-wrap gap-[0.375rem]">
				{dataArray.map((data) => (
					<div
						...
						<Label label={data.label.toString()} className={cn("text-lynch-500", data?.className)} />
					</div>
				))}
			</div>
			<div>
				{payment.status === PaymentStatusType.PENDING && (
					<PaymentValidation label="CONFIRMER" id={payment.ref} isMobile />
				)}
			</div>
		</div>
	)
