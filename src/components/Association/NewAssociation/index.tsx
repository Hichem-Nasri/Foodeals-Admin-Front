...
export const NewAssociation: React.FC<NewAssociationProps> = ({ partnerDetails }) => {
  const [countryCode, setCountryCode] = useState(countryCodes[0].value)
  const associationInformation = useForm<z.infer<typeof associationInformationSchema>>({
    resolver: zodResolver(associationInformationSchema),
    mode: "onBlur",
    defaultValues: {
      ...(partnerDetails ? partnerDetails : defaultAssociationInformationData),
    },
  })
  ...
  const onSubmitPartnerInfo = (data: z.infer<typeof associationInformationSchema>) => { }
  ...
  const onSaveData = () => {
    ...
    console.log(associationInformation.getValues(), partnerEngagement.getValues())
  }
  ...
  return (
    <div className="flex flex-col gap-[0.625rem] w-full lg:px-3 lg:mb-0 mb-20 overflow-auto">
      <TopBar
        ...
        secondaryButtonDisabled={associationInformation.formState.isValid}
      ...
      <div className="flex flex-col gap-[1.875rem] h-full w-full">
        ...
        />
        <FormEngagement form={partnerEngagement} onSubmit={onSubmitEngagement} disabled={!partnerDetails} />
      </div>
    </div>
  )
}
