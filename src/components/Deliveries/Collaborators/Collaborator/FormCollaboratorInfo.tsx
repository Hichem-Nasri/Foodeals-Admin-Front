...
import { CollaboratorDeliverySchema } from "@/types/DeliverySchema"
import { PartnerOptions } from "@/lib/utils"
import { AvatarProfile } from "@/components/AvatarProfile"
import { Label } from "@/components/Label"

interface FormCollaboratorInfoProps {
	form: UseFormReturn<z.infer<typeof CollaboratorDeliverySchema>>
	onSubmit: (data: z.infer<typeof CollaboratorDeliverySchema>) => void
  ...
  const adaptOptions = PartnerOptions.map((option) => ({ key: option.id, label: option.name }))
	return (
		<Accordion
			...
			<AccordionItem value="FormCollaboratorInfo" className="text-lynch-400 text-[1.375rem] font-normal">
								...
								<div className="flex flex-col gap-[1.875rem]">
									...
									<div className="flex lg:flex-row flex-col items-start gap-3">
										...
										<InputFieldForm
											...
											disabled={disabled}
										/>
										<InputFieldForm label="Rôle" name="role" control={control} placeholder="Rôle" disabled={disabled} />
									</div>
									<div className="flex lg:flex-row flex-col items-start gap-3">
										<InputPhoneField
											control={control}
