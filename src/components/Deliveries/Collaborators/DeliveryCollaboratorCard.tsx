...
import { DeliveryCollaboratorsType, DeliveryPaymentsType } from "@/types/deliveries"
...
export const DeliveryCollaboratorCard: React.FC<DeliveryCollaboratorCardProps> = ({ collaborator }): JSX.Element => {
	...
	const actions: ActionType[] = [
		{
			actions: (id) => router.push(appRoutes.deliveryCollaboratorDetails.replace(":id", id)),
			icon: Eye,
			label: "Voir",
		},
	]
			...
			<div className="flex justify-between gap-[0.375rem]">
				<div className="flex gap-[0.375rem]">
					<Avatar className="size-[2.875rem] shrink-0">
						<AvatarImage className="" src={collaborator.collaborator.avatar} />
						<AvatarFallback>{collaborator.collaborator.name[0].toUpperCase()}</AvatarFallback>
					</Avatar>
					<div className="flex flex-col gap-1">
						<Label label={collaborator.collaborator.name} className="text-sm font-normal text-lynch-950" />
						<Label label={collaborator.city} className="text-xs font-medium text-primary" />
						<div className="flex items-center gap-2 text-lynch-500">
							<CalendarClock size={18} />
							<Label label={collaborator.date.toLocaleDateString()} className="text-xs font-medium text-lynch-500" />
						</div>
					</div>
				</div>
