...
import { Banknote, Calendar, CalendarClock, CirclePercent, Coins } from "lucide-react"
...
export const DeliveryPaymentCard: React.FC<DeliveryPaymentCardProps> = ({ payment }): JSX.Element => {
	const dataArray = [
		{
			label: payment.month,
			icon: Calendar,
		...
		<div className="flex flex-col gap-3 bg-white p-3 rounded-[20px]">
				...
				<div className="flex gap-[0.375rem]">
					<div className="flex flex-col gap-1">
						<div className="flex items-center gap-2 text-lynch-500">
							...
							<Label label={payment.month} className="text-xs font-medium text-lynch-500" />
						</div>
					</div>
				</div>
			...
			<div>
				{payment.status === PaymentStatusType.PENDING && (
					<PaymentValidation label="CONFIRMER" id={payment.id} isMobile />
				)}
			</div>
		</div>
	)
