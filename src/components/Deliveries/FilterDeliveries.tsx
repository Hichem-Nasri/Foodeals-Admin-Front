...
export const FiltersDeliveries: FC<FiltersDeliveriesProps> = ({ data, form, table }) => {
  const router = useRouter()
  return (
    <div className="flex justify-between w-full rounded-[18px] lg:bg-white">
      <div className="flex lg:hidden items-center justify-between w-full">
        <h2 className="font-medium text-[1.375rem] text-lynch-950">Liste des livraisons</h2>
        <FormFilter />
      </div>
      ...
      <div className="lg:flex hidden gap-3 p-2">
        <CustomButton
          ...
          onClick={() => router.push(appRoutes.newDelivery.replace(":id", "new"))}
        />
        <CustomButton
          size="sm"
