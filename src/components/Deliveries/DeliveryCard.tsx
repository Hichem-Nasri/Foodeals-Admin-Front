...
import { ArchiveRestore, Building, CalendarClock, Eye, Frame, Mail, PhoneCall, Users } from "lucide-react"
...
export const DeliveryCard: React.FC<DeliveryCardProps> = ({ delivery }): JSX.Element => {
	...
	const actions: ActionType[] = [
		{
			actions: () => router.push(appRoutes.newDelivery.replace(":id", delivery.id)),
			icon: Eye,
			label: "Voir",
		},
		{
			actions: () => router.push(appRoutes.deliveryCollaborator.replace(":id", delivery.id)),
			icon: Users,
			label: "Collaborateurs",
		},
		{
			actions: () => router.push(appRoutes.deliveryPayment.replace(":id", delivery.id)),
			icon: Users,
		...
		<div className="flex flex-col gap-3 bg-white p-3 rounded-[20px]">
			<div className="flex justify-between gap-[0.375rem] cursor-pointer">
				<div className="flex gap-[0.375rem]">
					<Avatar className="size-[2.875rem] shrink-0">
						...
						<AvatarFallback>{delivery.partner.name[0].toUpperCase()}</AvatarFallback>
					</Avatar>
					<div className="flex flex-col gap-1">
						<Label label={delivery.partner.name} className="text-sm font-normal text-lynch-950" />
						<Label label={delivery.city} className="text-xs font-medium text-primary" />
						<div className="flex items-center gap-2 text-lynch-500">
							<CalendarClock size={18} />
							<Label label={delivery.createdAt.toLocaleDateString()} className="text-xs font-medium text-lynch-500" />
						</div>
					</div>
				</div>
			...
			<div className="flex flex-wrap gap-[0.375rem]">
				{dataArray.map((data) => (
					<div
						key={data.label}
						className={cn("flex gap-[0.375rem] bg-lynch-100 text-lynch-500 rounded-full py-[0.375rem] px-3")}>
						<data.icon size={18} key={data.label} />
						<Label label={data.label.toString()} className={cn("text-lynch-500")} />
					</div>
				))}
			</div>
			<span className="h-[1px] w-full bg-lynch-100" />
