...
export const DataTableSkeleton: FC<DataTableSkeletonProps> = ({ columnCount, rowCount }) => {
    return (
                ...
                <TableHeader>
                    <TableRow>
                        {Array.from({ length: columnCount }).map((_, index) => (
                            <TableHead key={index} className={index === 0 ? 'min-w-48' : 'min-w-40'}>
                                <Skeleton
                    ...
                    {Array.from({ length: rowCount }).map((_, rowIndex) => (
                        <TableRow key={rowIndex}>
                            {Array.from({ length: columnCount }).map((_, cellIndex) => (
                                <TableCell key={cellIndex} className={cellIndex === 0 ? 'min-w-48' : 'min-w-40'}>
                                    <Skeleton
